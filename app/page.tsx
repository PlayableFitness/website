    ...
    <header className="sticky top-0 z-50 border-b border-black/10 bg-[#f7f7f2]/85 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <nav className="hidden items-center gap-8 text-xs font-bold uppercase tracking-[0.12em] text-black/70 md:flex">
          <Link href="/" className="text-black underline decoration-[#00D1B2] decoration-2 underline-offset-8">Home</Link>
...
function PlayerCardStack() {
  const cards = [
    ...
    { src: "/cards/card_Jaydon.png", className: "z-20 -translate-x-16 translate-y-8 -rotate-6 scale-95 opacity-95" },
    { src: "/cards/card_David.png", className: "z-10 translate-x-16 translate-y-10 rotate-6 scale-95 opacity-95" },
  ];
...
function SystemStrip() {
  const stats = [
    ["REAL MOVEMENT", "Visible Progress", "Runs, workouts, recovery and consistency become visible development."],
    ["GAME DYNAMICS", "Instant Feedback", "Your player evolves immediately through visible skills, ratings and progress."],
    ["PLAYER EVOLUTION", "Build Yourself", "Short-term motivation drives long-term physical and personal development."],
    ["SEASON ZERO", "Now Forming", "Become part of the first generation shaping a new world of movement and progression."],
  ];
    ...
    <div className="relative z-10 mx-auto -mt-10 max-w-7xl px-6">
      <div className="grid overflow-hidden rounded-2xl border border-black/10 bg-white/90 shadow-xl shadow-black/5 backdrop-blur-xl md:grid-cols-4">
        {stats.map(([label, value, sub]) => (
          <div key={label} className="border-black/10 p-6 md:border-r last:md:border-r-0">
            <p className="text-[11px] font-black uppercase tracking-[0.16em] text-black/55">{label}</p>
            <p className="mt-2 text-3xl font-black uppercase tracking-tight text-black">{value}</p>
    ...
    <section className="border-t border-black/10 bg-[#f7f7f2]">
      <div className="mx-auto max-w-7xl px-6 py-16 md:py-24">
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-xs font-black uppercase tracking-[0.28em] text-black/45">{eyebrow}</p>
          <h2 className="mt-4 text-4xl font-black uppercase leading-[0.95] tracking-tight text-black md:text-6xl">{title}</h2>
...
function PlayerStatusSection() {
  const stages = [
    ["Level 01", "Rookie", "42 OVR", "Every player starts somewhere. Movement and consistency develop your progress step by step.", "/players/stufe1.jpg"],
    ["Level 02", "Starter", "58 OVR", "Your stats rise, your progress becomes visible and your player identity starts to evolve.", "/players/stufe2.jpg"],
    ["Level 03", "Elite", "74 OVR", "New unlocks, higher status and visible development reflect your long-term progress.", "/players/stufe3.jpg"],
    ["Level 04", "Pro", "91 OVR", "Top players unlock special styles, new opportunities and higher visibility.", "/players/stufe4.jpg"],
  ];
    ...
    >
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        {stages.map(([stage, title, rating, text, image]) => (
          <div key={title} className="group overflow-hidden rounded-3xl border border-black/10 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:shadow-black/10">
            <div className="relative aspect-[9/16] overflow-hidden bg-[#ecece5]">
              <img src={image} alt={title} className="h-full w-full object-cover object-[center_30%] transition duration-700 group-hover:scale-105" />
              <div className="absolute left-4 top-4 rounded-xl bg-[#00D1B2] px-3 py-2 text-xs font-black uppercase text-black">{rating}</div>
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent p-5">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-white/55">{stage}</p>
                <h3 className="mt-2 text-3xl font-black uppercase text-white">{title}</h3>
              </div>
            </div>
            <div className="p-6">
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-2">
        <div className="rounded-3xl border border-black/10 bg-white p-8">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-black/45">Jerseys & Brands</p>
          <h3 className="mt-3 text-2xl font-black uppercase text-black">Your style</h3>
          <p className="mt-4 text-black/65">As you progress, you unlock new jerseys, sports brands and individual styles for your player.</p>
        </div>

        <div className="rounded-3xl border border-black/10 bg-white p-8">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-black/45">Identity & Status</p>
          <h3 className="mt-3 text-2xl font-black uppercase text-black">What you stand for</h3>
          <p className="mt-4 text-black/65">Your progress becomes visible through status, styles, brands and the identity your player carries into the world.</p>
        </div>
      </div>
    </Section>
  );
    ...
    >
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="relative overflow-hidden rounded-3xl border border-black/10 bg-white p-8 md:p-10">
          <img src="/logik/team.jpg" alt="Team" className="absolute inset-0 h-full w-full object-cover opacity-20" />
  ...
  return (
    <section className="bg-[#f7f7f2] px-6 pb-16 md:pb-24">
      <div className="mx-auto grid max-w-7xl gap-6 md:grid-cols-2">
        <div className="rounded-3xl border border-black/10 bg-white p-8 md:p-10">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-black/55">Season Zero</p>
          <h2 className="mt-4 text-4xl font-black uppercase leading-none tracking-tight md:text-5xl">Your progress starts now.</h2>
          <p className="mt-6 max-w-md text-black/65">Join the first people connecting real movement, visible progress and true game motivation.</p>
          <Link href="/players" className="mt-8 inline-flex rounded-xl bg-[#00D1B2] px-6 py-4 text-xs font-black uppercase tracking-[0.14em] text-black">
            Start your progression →
          </Link>
        </div>

        <div className="relative overflow-hidden rounded-3xl border border-black/10 bg-white p-8 md:p-10">
          <img src="/vision/vision_05.jpg" alt="Season Zero" className="absolute inset-0 h-full w-full object-cover object-[center_25%] opacity-35" />
          <div className="relative max-w-md">
            <p className="text-xs font-black uppercase tracking-[0.22em] text-black/55">Season Zero</p>
            <h2 className="mt-4 text-4xl font-black uppercase leading-none tracking-tight md:text-5xl">The first generation is forming.</h2>
            <p className="mt-6 text-black/70">Athletes, creators, teams, gyms and brands are shaping the first real-world progression platform.</p>
            <Link href="/partners" className="mt-8 inline-flex rounded-xl bg-black px-6 py-4 text-xs font-black uppercase tracking-[0.14em] text-white">
              Build with us →
            </Link>
          </div>
        </div>
        ...
        <div className="relative mx-auto grid min-h-[72vh] max-w-7xl items-center gap-10 px-6 py-12 md:min-h-[78vh] md:py-20 md:grid-cols-[1.1fr_0.9fr]">
          <div className="max-w-2xl">
            ...
            <h1 className="text-6xl font-black uppercase leading-[0.88] tracking-tight text-black md:text-8xl xl:text-[7.4rem]">
              ...
              Play.
              <br />
              <span className="text-[#00D1B2] drop-shadow-sm">Progress.</span>
            </h1>

            <p className="mt-8 max-w-lg text-xl leading-relaxed text-black/75">
              Real movement develops your player through visible progress, identity, teams and long-term development.
            </p>
